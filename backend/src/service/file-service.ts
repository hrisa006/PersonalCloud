...
export default class FileService {
  ...
  public async fileUpload(
    ...
    const result = await this.handleBusboyFileUpload(
      userId + "/" + reqFilePath,
      req
    );
  ...
  public async getFilePath(userId: string, req: Request): Promise<string> {
    ...
    const internalFilePath = this.getInternalFilePathByOwner(
      ownerId,
      userId,
      reqFilePath
    );
  ...
  public async updateFile(
    ...
    const result = await this.handleBusboyFileUpload(
      path.dirname((ownerId === "" ? userId : ownerId) + "/" + reqFilePath),
      req
    );
    result.path = result.path.split("/").slice(1).join("/");

    await this.fileRepo.updateFile(
  ...
  public async shareFileWithUser(
    ...
    console.log(user.id);
    return this.fileRepo.shareFileWithUser(
      filePath,
      ownerId,
      user.id,
      permission
  ...
  public async getUsersFileIsSharedWith(
    ownerId: string,
    req: Request
  ): Promise<Users[]> {
  ...
  public async updateSharedPermission(
    ownerId: string,
    req: Request
  ): Promise<SharedFiles> {
    ...
    return this.fileRepo.updateSharedPermission(
      filePath,
      ownerId,
      userId,
      permission
  ...
  public async getUserFilePermission(
    ownerId: string,
    userId: string,
    filePath: string
  ...
  private async assertCanReadFile(
    ...
    const permission = await this.getUserFilePermission(
      ownerId,
      userId,
      filePath
    );
    if (
      ...
      (permission.permissions !== PermissionType.READ &&
        permission.permissions !== PermissionType.WRITE)
    ) {
      throw new BadRequestError(
        "User does not have read permission for this file"
      );
    }
  }

  private async assertCanWriteFile(
    userId: string,
    filePath: string,
    ownerId: string
  ): Promise<void> {
    ...
    if (!permission || permission.permissions !== PermissionType.WRITE) {
      throw new BadRequestError(
        "User does not have write permission for this file"
      );
    }
  }
  ...
  private handleBusboyFileUpload(
    filePath: string,
    req: Request
  ): Promise<FileUploadResponseDto> {
    return new Promise((resolve, reject) => {
      ...
      bb.on(ERROR, (err: any) =>
        reject(new BadRequestError(err.message || "Error in file upload"))
      );
  ...
  private getRootPath(): string {
    ...
    for (let i = 0; i < DIR_BACK_LEVELS; i++) result = path.dirname(result);
    return result;
  }
  ...
  private getInternalFilePathByOwner(
    ownerId: string,
    userId: string,
    reqFilePath: string
  ) {
